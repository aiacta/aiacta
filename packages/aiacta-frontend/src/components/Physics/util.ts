...
export function createShape(mesh: Mesh, faceIndices: number[]) {
  const { position } = mesh.geometry.attributes;
  const index =
    mesh.geometry.index ??
    new BufferAttribute(
      new Uint16Array(new Array(position.count).fill(null).map((_, i) => i)),
      1,
      false,
    );
  const faces: number[][] = [];

  const vertices = faceIndices
    .flatMap((i) => {
      faces.push([i * 3, i * 3 + 1, i * 3 + 2]);
      return [
        new Vector3().fromBufferAttribute(position, index.getX(i * 3)),
        new Vector3().fromBufferAttribute(position, index.getX(i * 3 + 1)),
        new Vector3().fromBufferAttribute(position, index.getX(i * 3 + 2)),
      ];
    })
    .map((v) => new Vec3(v.x, v.y, v.z));
